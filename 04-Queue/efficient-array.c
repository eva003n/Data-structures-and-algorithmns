...
int enqueue(Queue *q, bool (*ptrIsFull)(Queue *), int element)
{
    ...
    int rear = q->size++; // update the size also
    q->arr[rear] = element;
    return element;
};
...
int dequeue(Queue *q, bool (*ptrIsEmpty)(Queue *))
{
    ...
    q->front++;// problem queue gets full early since are not actually deleting items
  //  q->size--;(no decreasing size since we are not deleting anything)
