...
int enqueue(Queue *q, bool (*ptrIsFull)(Queue *), int element)
{
    ...
    int rear = q->front + (q->size++); // update the size also
    q->arr[rear] = element;
    return element;
};
...
int dequeue(Queue *q, bool (*ptrIsEmpty)(Queue *))
{
    ...
    // remove by shifting all elements to the left except first(simulate dequeue)
    int j; // track rightmost element to shift to left
    int deletedElement = q->arr[0];
